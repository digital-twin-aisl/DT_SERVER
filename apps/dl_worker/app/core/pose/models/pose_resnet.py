...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, cfg=None, **kwargs):
        self.inplanes = 64
        self.deconv_with_bias = deep_getattr(cfg, "POSE_RESNET.DECONV_WITH_BIAS", False)
        ...
        self.deconv_layers = self._make_deconv_layer(
            deep_getattr(cfg, "POSE_RESNET.NUM_DECONV_LAYERS", 3),
            deep_getattr(cfg, "POSE_RESNET.NUM_DECONV_FILTERS", [256, 256, 256]),
            deep_getattr(cfg, "POSE_RESNET.NUM_DECONV_KERNELS", [4, 4, 4]),
        )

        self.final_layer = nn.Conv2d(
            in_channels=deep_getattr(cfg, "POSE_RESNET.NUM_DECONV_FILTERS", [256, 256, 256])[-1],
            out_channels=deep_getattr(cfg, "POSE_RESNET.NUM_JOINTS", 15),
            kernel_size=deep_getattr(cfg, "POSE_RESNET.FINAL_CONV_KERNEL", 1),
            stride=1,
            padding=1 if deep_getattr(cfg, "POSE_RESNET.FINAL_CONV_KERNEL", 1) == 3 else 0,
        )
...
def get_pose_net(cfg=None, is_train=False, **kwargs):
    num_layers = deep_getattr(cfg, "POSE_RESNET.NUM_LAYERS", 50)
