...
from .backbones.cheby_graph_conv import graph_conv_cheby
...
class Pose2Mesh(nn.Module):
    ...
    def forward(self, x):
        self.graph_L = [self.graph_L[graph_i].cuda() if not self.graph_L[graph_i].is_cuda else self.graph_L[graph_i] for graph_i in range(len(self.graph_L))]
        self.graph_size=self.graph_L[-1].shape[0]
        
        x = x.view(-1, self.graph_size, self.num_joint_input_chan)
