...
def export_tensorrt(model, output_dir, cfg, mode="fp16"):
    ...
    backbone_x = torch.ones((batch_size, 3, cfg.NETWORK.IMAGE_SIZE[1], cfg.NETWORK.IMAGE_SIZE[0]), device=device)
    root_v2v_net_x = torch.ones((batch_size, 1, cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[0], cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[1], cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[2]), device=device)
    pose_v2v_net_x = torch.ones((1, cfg.NETWORK.NUM_JOINTS, cfg.PICT_STRUCT.CUBE_SIZE[0], cfg.PICT_STRUCT.CUBE_SIZE[1], cfg.PICT_STRUCT.CUBE_SIZE[2]), device=device)
    ...
    if mode == "int8":
        ...
        root_v2v_net_trt = torch2trt(root_v2v_net, [root_v2v_net_x], int8_mode=True, use_onnx=True)
        pose_v2v_net_trt = torch2trt(pose_v2v_net, [pose_v2v_net_x], int8_mode=True, use_onnx=True)
...
def load_tensorrt_model(model, ckpt_dir, cfg, mode="fp16"):
    ...
    batch_size = cfg.BATCH_SIZE
    num_views = cfg.NUM_VIEWS
    tensorrt_dir = os.path.join(ckpt_dir.split(".")[0], f'engine_{mode}_{batch_size}_{num_views}')
