...
def export_tensorrt(model, output_dir, cfg, mode="fp16"):
    ...
    backbone_x = torch.ones((batch_size, 3, cfg.NETWORK.IMAGE_SIZE[1], cfg.NETWORK.IMAGE_SIZE[0])).cuda()
    root_v2v_net_x = torch.ones((batch_size, 1, cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[0], cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[1], cfg.MULTI_PERSON.INITIAL_CUBE_SIZE[2])).cuda()
    ...
    if mode == "fp16":
        backbone_trt = torch2trt(backbone, [backbone_x], fp16_mode=False, use_onnx=True)
